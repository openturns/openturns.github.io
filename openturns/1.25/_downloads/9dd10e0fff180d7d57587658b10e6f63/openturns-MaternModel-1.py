...
if "MaternModel" == "ExponentialModel":
    covarianceModel = ot.ExponentialModel([0.5], [5.0])
elif "MaternModel" == "GeneralizedExponential":
    covarianceModel = ot.GeneralizedExponential([2.0], [3.0], 1.5)
elif "MaternModel" == "ProductCovarianceModel":
    amplitude = [1.0]
    scale1 = [4.0]
    scale2 = [4.0]
...
elif "MaternModel" == "RankMCovarianceModel":
    variance = [1.0, 2.0]
    basis = ot.LinearBasisFactory().build()
    covarianceModel = ot.RankMCovarianceModel(variance, basis)
elif "MaternModel" == "StationaryFunctionalCovarianceModel":
