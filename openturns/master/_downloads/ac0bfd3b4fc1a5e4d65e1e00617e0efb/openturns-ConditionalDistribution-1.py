...
title = None
if "ConditionalDistribution" == "Bernoulli":
    distribution = ot.Bernoulli(0.7)
elif "ConditionalDistribution" == "Binomial":
    distribution = ot.Binomial(5, 0.2)
elif "ConditionalDistribution" == "Hypergeometric":
    distribution = ot.Hypergeometric(10, 4, 7)
elif "ConditionalDistribution" == "CumulativeDistributionNetwork":
    coll = [ot.Normal(2),ot.Dirichlet([0.5, 1.0, 1.5])]
    distribution = ot.CumulativeDistributionNetwork(coll, ot.BipartiteGraph([[0,1], [0,1]]))
elif "ConditionalDistribution" == "Histogram":
    distribution = ot.Histogram([-1.0, 0.5, 1.0, 2.0], [0.45, 0.4, 0.15])
elif "ConditionalDistribution" == "KernelMixture":
    kernel = ot.Uniform()
    sample = ot.Normal().getSample(5)
    bandwidth = [1.0]
    distribution = ot.KernelMixture(kernel, bandwidth, sample)
elif "ConditionalDistribution" == "MaximumDistribution":
    coll = [ot.Uniform(2.5, 3.5), ot.LogUniform(1.0, 1.2), ot.Triangular(2.0, 3.0, 4.0)]
    distribution = ot.MaximumDistribution(coll)
elif "ConditionalDistribution" == "Multinomial":
    distribution = ot.Multinomial(5, [0.2])
elif "ConditionalDistribution" == "RandomMixture":
    coll = [ot.Triangular(0.0, 1.0, 5.0), ot.Uniform(-2.0, 2.0)]
    weights = [0.8, 0.2]
    cst = 3.0
    distribution = ot.RandomMixture(coll, weights, cst)
elif "ConditionalDistribution" == "SmoothedUniform":
    distribution = ot.SmoothedUniform(-1.0, 10.0, 1.0)
elif "ConditionalDistribution" == "TruncatedDistribution":
    distribution = ot.TruncatedDistribution(ot.Normal(2.0, 1.5), 1.0, 4.0)
elif "ConditionalDistribution" == "UserDefined":
    distribution = ot.UserDefined([[1.0], [2.0], [3.0]], [0.4, 0.5, 1.0])
elif "ConditionalDistribution" == "ZipfMandelbrot":
    distribution = ot.ZipfMandelbrot(10, 2.5, 0.3)
elif "ConditionalDistribution" == "Normal":
