...
if "Pareto" == "Bernoulli":
...
elif "Pareto" == "Binomial":
    distribution = ot.Binomial(5, 0.2)
elif "Pareto" == "Hypergeometric":
    distribution = ot.Hypergeometric(10, 4, 7)
elif "Pareto" == "CumulativeDistributionNetwork":
    coll = [ot.Normal(2),ot.Dirichlet([0.5, 1.0, 1.5])]
    distribution = ot.CumulativeDistributionNetwork(coll, ot.BipartiteGraph([[0,1], [0,1]]))
elif "Pareto" == "Histogram":
    distribution = ot.Histogram([-1.0, 0.5, 1.0, 2.0], [0.45, 0.4, 0.15])
elif "Pareto" == "KernelMixture":
    kernel = ot.Uniform()
    sample = ot.Normal().getSample(5)
    bandwidth = [1.0]
    distribution = ot.KernelMixture(kernel, bandwidth, sample)
elif "Pareto" == "MaximumDistribution":
    coll = [ot.Uniform(2.5, 3.5), ot.LogUniform(1.0, 1.2), ot.Triangular(2.0, 3.0, 4.0)]
    distribution = ot.MaximumDistribution(coll)
elif "Pareto" == "Multinomial":
    distribution = ot.Multinomial(5, [0.2])
elif "Pareto" == "LinearCombinationDistribution":
    ...
    if hasattr(ot, "Pareto"):
        distribution = ot.Pareto()
    else:
        distribution = otexp.Pareto()
