...
.. plot::
    ...
    elif ot.TensorizedCovarianceModel().__class__.__name__ == 'GeneralizedExponential':
        covarianceModel = ot.GeneralizedExponential([2.0], [3.0], 1.5)
    elif ot.TensorizedCovarianceModel().__class__.__name__ == 'ProductCovarianceModel':
        amplitude = [1.0]
        scale1 = [4.0]
        scale2 = [4.0]
    ...
    elif ot.TensorizedCovarianceModel().__class__.__name__ == 'RankMCovarianceModel':
        variance = [1.0, 2.0]
        basis = ot.LinearBasisFactory().build()
        covarianceModel = ot.RankMCovarianceModel(variance, basis)
    elif ot.TensorizedCovarianceModel().__class__.__name__ == 'StationaryFunctionalCovarianceModel':
