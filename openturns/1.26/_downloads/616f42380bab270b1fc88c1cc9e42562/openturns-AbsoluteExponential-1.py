...
if "AbsoluteExponential" == "ExponentialModel":
    covarianceModel = ot.ExponentialModel([0.5], [5.0])
elif "AbsoluteExponential" == "GeneralizedExponential":
    covarianceModel = ot.GeneralizedExponential([2.0], [3.0], 1.5)
elif "AbsoluteExponential" == "ProductCovarianceModel":
    amplitude = [1.0]
    scale1 = [4.0]
    scale2 = [4.0]
...
elif "AbsoluteExponential" == "RankMCovarianceModel":
    variance = [1.0, 2.0]
    basis = ot.LinearBasisFactory().build()
    covarianceModel = ot.RankMCovarianceModel(variance, basis)
elif "AbsoluteExponential" == "StationaryFunctionalCovarianceModel":
